                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_s and pygame.key.get_mods() & pygame.KMOD_CTRL:
                        ...
                        if not os.path.exists(folder):
                            os.makedirs(folder)
                        maps = [f for f in os.listdir(folder) if f.startswith("map") and f.endswith(".json")]
                        index = int(maps[-1][3:-5]) if maps else 0
                        map_path = f'maps/map{index + 1}.json'
                        self.tilemap.save(map_path, (17, 12))
